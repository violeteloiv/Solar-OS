...
pub unsafe fn print_mmap_sections(info: *const MultibootInfo) {
    ...
    println!("Bootloader Name: {}", boot_loader_str);
    println!("Available Memory Segments...");
    println!("# Memory Mappable Addresses: {num_mmap_addrs}");
    ...
    for entry in (*info).get_mmap_addrs() {
        ...
        println!("size: {size}, len: {len} KB, addr: {addr:#04X}");
    }

    println!("total_length: {} MB", total_length as f32 / 1024.0 / 1024.0);
}