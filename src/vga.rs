...
pub static TERMINAL_WRITER: WriterWrapper = WriterWrapper {
...
pub struct WriterWrapper {
    pub inner: UnsafeCell<TerminalWriter>,
}

unsafe impl Sync for WriterWrapper {}
...
impl TerminalWriter {
    ...
    pub fn init() -> &'static TerminalWriter {
        let color = unsafe { (*TERMINAL_WRITER.inner.get()).terminal_color.load(Ordering::Relaxed) };
        for y in 0..VGA_HEIGHT {
            for x in 0..VGA_WIDTH {
                let index = y * VGA_WIDTH + x;
                unsafe {
                    *(*TERMINAL_WRITER.inner.get()).terminal_buffer.add(index) = vga_entry(b' ', color);
                }
            }
        }
        unsafe { &(*TERMINAL_WRITER.inner.get()) }
    }
